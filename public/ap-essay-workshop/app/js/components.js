...
export function WritingArea(rung, activityId, responseId, options = {}) {
  ...
  textarea.addEventListener('input', () => {
    ...
    debounce = setTimeout(() => {
      if (uid()) saveResponse(uid(), rung, activityId, responseId, textarea.value);
    }, 2000);
...
export function EditableTable(rung, tableId, headers, rows, modelData = null) {
    ...
    row.forEach((cell, ci) => {
      ...
      if (isEditable) {
        td.addEventListener('blur', () => {
          // Collect all table data and save
          const data = [];
          ...
          if (uid()) saveTableData(uid(), rung, tableId, data);
        });
      }
      tr.appendChild(td);
    });
  ...
  container.loadData = (data) => {
    ...
    editableCells.forEach((cell, i) => {
      if (flat[i]) cell.textContent = flat[i];
    });
  };
